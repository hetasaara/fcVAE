...
class VAE(nn.Module): # needs n_labels as a parameter
    ...
    def loss(self, reconst_x, x, z_mu, z_var, n_head: int):
        # reconstruction loss
        #print(x)
        #recon_loss = F.binary_cross_entropy(reconst_x, x, reduction='sum')
        #x_long = x.type(torch.LongTensor)
        #print(x_long.size())
        #recon_loss = F.cross_entropy(reconst_x, x_long, reduction='sum')
        #print(recon_loss)
        #recon_loss = nn.BCELoss(reconst_x, x).sum(-1)
        #-torch.sum(Normal()) # if normal distribution, generative model should output mean and var?
        #-torch.sum(Normal(x_sample).log_prob(x), dim=1)
